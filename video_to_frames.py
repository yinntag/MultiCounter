...
parser.add_argument('--root', default="./PreTrainData/", help='Path to dataset root')
...
for split_dir in split_dirs:
    ...
    info = {'info': {'description': 'MultiRep Dataset', 'url': '1', 'version': '1', 'year': '2024',
                     'contributor': 'Yin Tang',
                     'data_created': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}}
    licenses = {'licenses': 'only for research'}
    categories = {'categories': [{'supercategory': 'object', 'id': 1, 'name': 'human_instance'}]}
    ...
    for video_sample in tqdm(video_list):
        video_path = os.path.join(split_dataset_root, str(video_sample), video_sample + '_augmented.mp4')
        anno_path = os.path.join(split_dataset_root, str(video_sample), video_sample + '_updated_.json')
        ...
        for instance in origin_anno:
            ...
            for index in range(0, length):
                if origin_anno[instance]['bbox'][index] == None:
                    anno_period.append(None)
                    continue
                else:
                    origin_anno[instance]['bbox'][index][0] = origin_anno[instance]['bbox'][index][0] * scale_w
                    origin_anno[instance]['bbox'][index][1] = origin_anno[instance]['bbox'][index][1] * scale_h
                    new_x1 = origin_anno[instance]['bbox'][index][0]
                    new_y1 = origin_anno[instance]['bbox'][index][1]
                    origin_anno[instance]['bbox'][index][2] = origin_anno[instance]['bbox'][index][2] * scale_w - new_x1
                    origin_anno[instance]['bbox'][index][3] = origin_anno[instance]['bbox'][index][3] * scale_h - new_y1
                period_sign = 0
                for period_index in range(0, len(origin_anno[instance]['period'])):
                    if index >= origin_anno[instance]['period'][period_index][0] and index <= \
                            origin_anno[instance]['period'][period_index][1]:
                        period_sign = 1
                        break
                anno_period.append(period_sign)
            anno.update({'bboxes': origin_anno[instance]['bbox']})
            anno.update({'periods': origin_anno[instance]['period']})
            anno.update({'periodicity': anno_period})
            anno.update({'period velocity': assign_period_lengths(length, origin_anno[instance]['period'])})
